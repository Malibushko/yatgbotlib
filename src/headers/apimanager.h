...
private:
    ...
    void assign_impl(T& value,const std::string& data) const {
        if constexpr (traits::is_string_type<T>)
                ...
                value = (utility::lowercase_compare(data,"true") ?  1 : 0);
            ...
            for (auto&& [name,path_or_id] : params) {
                if (auto path = fs::path(path_or_id);fs::exists(path)) {
                    if (path.filename().string().find(".pem") != std::string::npos)
                        items.push_back({"certificate",utility::get_file_bytes(fs::absolute(path)),'@'+path.string()});
                    else
                        items.push_back({path.filename(),utility::get_file_bytes(fs::absolute(path)),path.filename().string()});
                } else {
